...
class Trainer:
    ...
    def model_init(self):
        ...
        model = networks.DepthSepQKV(resize_shape=(self.opt.height, self.opt.width))
        lora_config = LoraConfig(
            r=14,  # 低秩维度，平衡效率和性能
            lora_alpha=20,  # 缩放因子，通常为 2*r
            target_modules=["query", "value", "projection"],  # 针对 ViT 层
            lora_dropout=0.05,  # dropout 防止过拟合
            bias="none",  # 不调整偏置
            task_type=None  # 自定义任务类型
        )
        ...
        if not self.opt.load_weights_folder:
            ...
            for model_name, weight_name in model_weights.items():
                ...
                for p in self.models[model_name].parameters():
    ...
    def run_epoch(self):
        ...
        for batch_idx, inputs in enumerate(self.train_loader):
            ...
            # 在backward前检查梯度（仅在需要时输出）
            # print(f"[DEBUG] Epoch {self.epoch}, Batch {batch_idx} - 检查梯度前状态")
    ...
    def suppress(self, inputs, outputs):
        ...
        for i, f_i in enumerate(self.opt.frame_ids[1:]):
            ...
            outputs[("decompose_A_warp", f_i)] = F.grid_sample(
    ...
    def compute_decompose_loss(self, inputs, outputs, losses):
        ...
        # 仅在需要时输出重构损失
        # print(f"[DEBUG] recons_loss: {recons_loss.item():.6f}")
        ...
        for f_i in self.opt.frame_ids:
            ...
            epsilon = 1e-8
            # 计算A分量和输入图像的梯度
            grad_A = self.nabla(outputs[("decompose_result", f_i)]["A"]) + epsilon
            grad_color = self.nabla(inputs[("color_aug", f_i, 0)]) + epsilon
            
            # 应用掩码并计算重投影损失
            masked_grad_color = grad_color * (1 - M)
            

            masked_grad_color = masked_grad_color + epsilon
        ...
        # 仅在需要时输出Retinex损失
        # print(f"[DEBUG] retinex_loss: {retinex_loss.item():.6f}")
        ...
        for f_i in self.opt.frame_ids:
            ...
            # 添加数值稳定性保护，避免梯度平方导致的数值爆炸
            epsilon = 1e-8
            grad_S = grad_S + epsilon
            smooth_S += torch.mean(grad_S ** 2)
        smooth_S /= self.num_input_frames
        
        # 仅在需要时输出平滑损失
        # print(f"[DEBUG] smooth_S: {smooth_S.item():.6f}")
    ...
    def compute_supressed_loss(self, inputs, outputs, losses):
        ...
        # # 检查NT-Xent损失
        # self.check_inf_and_exit(nt_xent_loss, "compute_supressed_loss() - NT-Xent损失计算", "nt_xent_loss")
        
        # losses["nt_xent_loss"] = nt_xent_loss
        # losses["loss"] += nt_xent_loss * self.opt.nt_xent_weight

        if self.epoch < self.start_depth_epoch:
            ...
            # 仅在需要时输出相似性损失
            # print(f"[DEBUG] sim_loss: {sim_loss.item():.6f}")
            
            losses["sim_loss"] = sim_loss
            losses["loss"] += sim_loss / (self.epoch + 1)
        ...
        for f_i in self.opt.frame_ids[1:]:
            ...
            # if mask_sum > 0:
            #     reprojection_loss += self.compute_reprojection_loss(
            #         outputs[("decompose_A_warp", f_i)],
            #         outputs[("decompose_result", 0)]["A"]
            #     ).sum() / mask_sum
        ...
        # 增强数值稳定性，使用更大的epsilon值防止除零
        norm_disp = disp / (mean_disp + 1e-3)  # 增大epsilon到1e-3
        # 添加clamp防止极端值
        norm_disp = torch.clamp(norm_disp, min=0.1, max=10.0)
        loss_disp_smooth = get_smooth_loss(norm_disp, color)  # 边缘感知的平滑损失
        
        # 仅在需要时输出视差平滑性损失
        # print(f"[DEBUG] disp_smooth_loss: {loss_disp_smooth.item():.6f}")
    ...
    def log(self, mode, inputs, outputs, losses):
        ...
        for l, v in losses.items():
            # 跳过loss tensor对象的self-check，因为它们在tensorboard中不会出现
            if l == "loss":
