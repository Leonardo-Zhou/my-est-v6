...
class Trainer:
    ...
    def model_init(self):
        ...
        self.models["reflection"] = networks.MaskedSpatioTemporalReflectionModule(
    ...
    def compute_supressed_loss(self, inputs, outputs, losses):
        ...
        mask_patch = self.models["reflection"].mask_embed
        for attn in self.models["reflection"].temporal_attention_maps:  # hooks 获取 attn (B, heads, N, points)
            attn = attn[:, :, 1:]
            residual = torch.abs(attn * mask_patch.unsqueeze(1).unsqueeze(1)).mean()  # 残留均值
            suppress_loss += residual
        suppress_loss /= len(self.models["reflection"].temporal_attention_maps)  # 平均层
        losses['suppress'] = suppress_loss
        losses["loss"] += suppress_loss * 0.05



        # 非高光区域相似性损失以及梯度损失，用于确保伪影不存在
        ...
        if self.epoch < self.opt.str_sim_epoch:
            ...
            for f_i in self.opt.frame_ids[1:]:
                ...
                M_flat = M.view(B, -1)
                thresholds = torch.quantile(M_flat, 0.90, dim=1, keepdim=True).unsqueeze(-1).unsqueeze(-1)
                M_spec = (M > thresholds).float()
                sim_loss += self.compute_reprojection_loss(
                    outputs[("suppressed", f_i)] * (1 - M_spec),
                    outputs[("A_warp", f_i)] * (1 - M_spec)
                ).mean()
                grad_loss += self.compute_reprojection_loss(
                    self.nabla(outputs[("suppressed", f_i)]) * (1 - M_spec),
                    self.nabla(outputs[("A_warp", f_i)]) * (1 - M_spec)
                ).mean()
            sim_loss /= self.num_input_frames
            losses["sim_loss"] = sim_loss
            losses["grad_loss"] = grad_loss
            losses["loss"] += sim_loss * 1.5 + grad_loss * 0.5

        if self.epoch >= self.opt.str_sim_epoch:
            reprojection_loss = torch.tensor(0.0, device=self.device)
            M_0 = outputs[("decompose_result", 0)]["M"]
            M_0_flat = M_0.view(B, -1)
            thresholds = torch.quantile(M_0_flat, 0.90, dim=1, keepdim=True).unsqueeze(-1).unsqueeze(-1)
            M_0_spec = (M_0 > thresholds).float()
            for f_i in self.opt.frame_ids[1:]:
                ...
                if mask_sum > 0:
                    ...
                    M = M_spec * M_0_spec
                    reprojection_loss += self.compute_reprojection_loss(
                        outputs[("A_warp", f_i)] * (1 - M),
                        outputs[("decompose_result", 0)]["A"] * (1 - M)
                    ).sum() / mask_sum
        ...
        if self.epoch >= self.opt.str_sim_epoch:
            ...
            M = outputs[("decompose_result", 0)]["M"]
            M_flat = M.view(B, -1)
            thresholds = torch.quantile(M_flat, 0.90, dim=1, keepdim=True).unsqueeze(-1).unsqueeze(-1)
            M_spec = (M > thresholds).float()
            loss_disp_smooth = get_smooth_loss(norm_disp, color, 1 - M_spec)
            loss_disp_smooth += (self.nabla(norm_disp) * M_spec).mean() * 0.05
    ...
    def log(self, mode, inputs, outputs, losses):
        ...
        if hasattr(self.models["reflection"], "sup_factors"):
            for i, factor in enumerate(self.models["reflection"].sup_factors):
                writer.add_scalar("suppress_factor/stage {}".format(i), factor, self.step)
